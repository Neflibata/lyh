...
class ApiIndexController extends PluginRestBaseController
{
     ...
     * 实时监控目录
     */
    public function getRegionDir()
    {
        $hk=new Haikang();
        $regions=Db::name('statistics_regions')->order('id asc')->select();
        $dir=Db::name('statistics_dir')->select()->toArray();
        foreach ($dir as $k=>$v){
            $dir[$k]['children']=Db::name('statistics_cameras')->where('dir_id',$v['id'])->field('id,dir_id,cameraName,regionIndexCode')->select();
        }
        $list=$hk->regions_sort($regions,$dir);//主菜单
        return zy_json_echo(true,$list);
    }
    /**
     * 实时监控目录2
     */
    public function getRegionDir2()
    {
        $hk=new Haikang();
        $where=[];
        $where['parentIndexCode']=['eq','root000000'];
        $where['name']=['notlike','%FD%'];
        $regions=Db::name('statistics_regions')->where($where)->order('id asc')->select()->toArray();
        $dir=Db::name('statistics_dir')->select()->toArray();
        $cameras=Db::name('statistics_cameras')->field('id,dir_id,cameraName,regionIndexCode')->select();
        $index=0;
        foreach ($regions as $k=>$v){
            foreach ($dir as $ke=>$val){
                if($v['indexCode']==$val['cameraIndexCode']){
                    $regions[$k]['child'][$index]=$val;
                    foreach ($cameras as $key=>$value){
                        if($val['id']==$value['dir_id']){
                            $regions[$k]['child'][$index]['child'][]=$value;
                        }
                    }
                    $index++;
                }
        ...
        return zy_json_echo(true,$regions);
    }
    /**
     * 实时监控目录3
     */
